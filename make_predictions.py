...
r = loss_jaccard(K.variable(predictions), 
                 K.variable(y_data_filtered)).eval(session = K.get_session())
...
for i in range(len(predictions)):
  ...
  plt.colorbar()
  plt.savefig('img/out_' + str(r[i][0]) + '_' +  str(i) + '.png')
  plt.close()
  
  image = np.swapaxes(y_data_filtered[i], 0, 2)
  plot = plt.imshow(image[0])
  plt.colorbar()
  plt.savefig('img/true_' + str(r[i][0]) + '_' +  str(i) + '.png')
  plt.close()
  
  image = np.swapaxes(x_data_filtered[i], 0, 2)
  plot = plt.imshow(image[0])
  plt.colorbar()
  plt.savefig('img/raw_' + str(r[i][0]) + '_' +  str(i) + '.png')
  plt.close()
