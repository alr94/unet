...
for filename in os.listdir(args.data):
  ...
  for i_key in range(n_steps):
    ...
    wirecalib   = root_numpy.hist2array(data_dir.Get('wireCalib'))
    energy      = root_numpy.hist2array(data_dir.Get('energy'))
    energycalib = root_numpy.hist2array(data_dir.Get('energyCalib'))
    
    cnnpred = np.zeros(wire.shape)
    try:
    ...
    cnnem       = root_numpy.hist2array(data_dir.Get('cnnem'))
    cnnmichel   = root_numpy.hist2array(data_dir.Get('cnnmichel'))
    cluem       = root_numpy.hist2array(data_dir.Get('cluem'))
    clumichel   = root_numpy.hist2array(data_dir.Get('clumichel'))
    x           = root_numpy.hist2array(data_dir.Get('hit_x'))
    y           = root_numpy.hist2array(data_dir.Get('hit_y'))
    z           = root_numpy.hist2array(data_dir.Get('hit_z'))
    closestSP   = root_numpy.hist2array(data_dir.Get('hit_z'))
    
    if args.datatype == 'MC':
      truth      = root_numpy.hist2array(data_dir.Get('truth'))
      truthcalib = root_numpy.hist2array(data_dir.Get('truthCalib'))
      trueenergy = root_numpy.hist2array(data_dir.Get('trueEnergy'))
